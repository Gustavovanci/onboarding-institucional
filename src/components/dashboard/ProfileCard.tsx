...
export default function ProfileCard({
  ...
  return (
    ...
    >
      {/* CORREÇÃO: O gradiente agora é dinâmico com base no tema escolhido */}
      <div className={`h-48 bg-gradient-to-r ${theme.primary} relative`}>
        ...
        <div className="absolute bottom-4 left-6 flex items-end gap-6">
          <div className="relative">
            <img
              src={user.photoURL || `https://ui-avatars.com/api/?name=${encodeURIComponent(user.displayName)}&size=160&background=ffffff&color=2b97d4`}
              alt={user.displayName}
              className="w-32 h-32 rounded-2xl border-4 border-white shadow-lg object-cover"
            />
            {/* CORREÇÃO: O emoji agora é dinâmico */}
