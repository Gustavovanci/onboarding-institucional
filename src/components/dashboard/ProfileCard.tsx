...
const cleanPhotoURL = (url: string | null | undefined): string | null => {
  ...
  // Se não tiver '=', retorna original (caso .../picture/0)
  return url;
};
...
export default function ProfileCard({
  ...
  const avatarParams = `?name=${encodeURIComponent(user.displayName)}&size=160&background=ffffff&color=2b97d4`;
  ...
  return (
    ...
    >
      ...
      {/* Conteúdo restante do card (bio, stats) permanece igual */}
      <div className="px-6 py-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            ...
            <div className="text-center">
                <div className="text-2xl font-bold text-brand-azure">{user.points || 0}</div>
                <div className="text-xs text-gray-500">Pontos</div>
            </div>
            <div className="text-center">
                <div className="text-2xl font-bold text-brand-teal">#{user.currentRank > 0 ? user.currentRank : 'N/A'}</div>
                <div className="text-xs text-gray-500">Ranking Geral</div>
            </div>
            <div className="text-center">
                <div className="text-2xl font-bold text-purple-600">#{user.instituteRank > 0 ? user.instituteRank : 'N/A'}</div>
                <div className="text-xs text-gray-500">No Instituto</div>
            </div>
            <div className="text-center">
                <div className="text-2xl font-bold text-yellow-600">{user.badges?.length || 0}</div>
